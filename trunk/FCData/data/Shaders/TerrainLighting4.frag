...
void main(void)
{
  vec4  a  = texture2D(alphamap, Texcoord),
  t1  = texture2D(base, Texcoord*scaleUVW),
  t2 = texture2D(texOne , Texcoord*scaleUVW),
  t3 = texture2D(texTwo , Texcoord*scaleUVW),
  t4 = texture2D(texThree , Texcoord*scaleUVW);

  float ix = 1.0 - a.x,
  iy = 1.0 - a.y,
  iz = 1.0 - a.z;
  vec4 l = a.x * t2 + ix * t1; //mix t1 with t2
  vec4 m = a.y * t3 + iy * l;  //then mix in t3
  gl_FragColor = a.z * t4 + iz * m; //finally mix in t4
}
*/
